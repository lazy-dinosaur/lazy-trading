  ...
  if (availableBalance && tradingFee) {
    try {
      ...
      if (isUSDTContract) {
        ...
        const finalPositionSizeUSDT = Math.min(
          positionSizeUSDT,
          maxPositionSize,
        );
        ...
        if (leverageInfo.leverageTier?.length) {
          console.log(
            "원래 티어 정보:",
            JSON.stringify(leverageInfo.leverageTier, null, 2),
          );
          ...
          const usdToKonVertedContractSize = contractSize / currentPrice; // USD를 BTC로 변환
          ...
          for (const tier of sortedTiers) {
            ...
            console.log(
              `티어 ${tier.tier}: maxNotional(BTC) ${tier.maxNotional}, maxNotional(USD) ${maxNotionalUsd}`,
            );

            if (
              tier.maxNotional &&
              usdToKonVertedContractSize <= tier.maxNotional
            ) {
              selectedTier = tier;
              ...
              finalContractSize = contractSize;
              console.log(
                `티어 ${tier.tier} 선택: maxNotional(BTC) ${tier.maxNotional}, maxNotional(USD) ${maxNotionalUsd}, 레버리지 ${finalLeverage}`,
              );
              break;
            }
          }
          ...
          if (!selectedTier) {
            ...
            // 최대 레버리지 적용 (리스크 관리는 주문 크기로 조절)
            finalLeverage =
              selectedTier?.maxLeverage ?? leverageInfo.maxLeverage;
            ...
            const maxNotionalUsd =
              (selectedTier?.maxNotional ?? 0) * currentPrice;
            finalContractSize = Math.min(contractSize, maxNotionalUsd);

            console.log(
              `최대 티어 선택: tier ${selectedTier?.tier}, maxNotional(BTC) ${selectedTier?.maxNotional}, maxNotional(USD) ${maxNotionalUsd}, 레버리지 ${finalLeverage}`,
            );
          }
        }
        ...
        if (margin > usableMargin) {
          ...
          const maxPossibleContractSize = Math.floor(
            usableMargin * finalLeverage,
          );
          ...
          if (maxPossibleContractSize < minAmount) {
            ...
            result.position = {
              ...
              fee: Number(ccxtInstance.costToPrecision(symbol, totalFee)),
              totalLoss: Number(
                ccxtInstance.costToPrecision(symbol, totalLoss),
              ),
            };
