...
export type LedgerType = "deposit" | "withdrawal" | "trade" | "fee" | "transaction" | "transfer" | "rebate" | "cashback" | "referral" | "other";
  ...
  try {
    ...
    for (const entry of ledgerEntries) {
      if (entry.datetime && typeof entry.usdValue === 'number' && !isNaN(entry.usdValue)) {
         ...
         if (entry.direction) {
           ...
           changeValue = entry.direction === 'out' ? -Math.abs(entry.usdValue) : Math.abs(entry.usdValue);
         } 
         // direction이 없는 경우 type을 통해 판단
           ...
           } else if (['deposit', 'funding', 'rebate', 'cashback'].includes(entry.type)) {
         ...
         console.log(`[DEBUG] 일별 변화량 추가: ${date} - ${changeValue} USD (${entry.type}, ${entry.direction || 'N/A'})`);
      }
    }
    ...
    for (let i = dates.length - 1; i >= 0; i--) {
      ...
      if (i < dates.length - 1) {
        ...
        console.log(`[DEBUG] ${date} 잔고 계산: ${balance} (변화량: ${change})`);
      }
...
export const calculateAdjustedReturn = async (
  ...
  try {
    ...
    const startDate = balanceHistory[0]?.time ? new Date(balanceHistory[0].time) : new Date();
    
    await Promise.all(Object.values(decryptedAccounts).map(async (account) => {
      try {
        // 계정의 원장 데이터 수집
        const ledgerEntries = await fetchAccountLedger(account, days);
        ...
        ledgerEntries.forEach(entry => {
          ...
          if (typeof entry.usdValue === 'number' && !isNaN(entry.usdValue)) {
            ...
            if (entryDate && startDate && entryDate >= startDate) {
              ...
              if (
                entry.type === 'deposit' || 
                (entry.direction === 'in' && ['deposit', 'funding', 'transfer'].includes(entry.type || ''))
              ) {
                // 항상 양수로 처리 (입금은 자산 증가)
                const depositAmount = Math.abs(entry.usdValue);
                totalDeposits += depositAmount;
                console.log(`[DEBUG] 입금 항목 추가: ${entry.datetime} - ${depositAmount} USD (${entry.type}, ${entry.direction || 'N/A'})`);
              }
              // 출금 판별 (type이 withdrawal이거나 direction이 out이고 출금 관련 유형인 경우)
              else if (
                entry.type === 'withdrawal' || 
                (entry.direction === 'out' && ['withdrawal', 'cashout', 'transfer'].includes(entry.type || ''))
              ) {
                // 항상 양수로 처리 (출금은 자산 감소지만 출금액으로 표시)
                const withdrawalAmount = Math.abs(entry.usdValue);
                totalWithdrawals += withdrawalAmount;
                console.log(`[DEBUG] 출금 항목 추가: ${entry.datetime} - ${withdrawalAmount} USD (${entry.type}, ${entry.direction || 'N/A'})`);
              }
            }
          }
        });
      } catch (error) {
        console.error(`[DEBUG] ${account.exchange} 계정의 입출금 계산 실패:`, error);
      }
    }));
    ...
    const periodReturn = finalAsset - initialAsset - (totalDeposits - totalWithdrawals);
